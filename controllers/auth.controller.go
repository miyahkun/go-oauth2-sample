...
func SignUpUser(ctx *gin.Context) {
	...
	if result.Error != nil && strings.Contains(result.Error.Error(), "UNIQUE constraint failed: users.email") {
		...
		ctx.JSON(http.StatusBadGateway, gin.H{"status": "error", "message": "Something bad happend"})
		return
	}
...
func SignInUser(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"status": "fali", "message": err.Error()})
		return
	}
...
func GoogleOAuth(ctx *gin.Context) {
	...
	if code == "" {
		ctx.JSON(http.StatusUnauthorized, gin.H{"status": "fail", "message": "Authorizaiton code not provided!"})
		return
	}
